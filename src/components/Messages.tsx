...
const Messages = ({
  ...
  useEffect(() => {
    if (id) {
      ...
      client?.subscribe(
        `a${id}`,
        "reaction_added",
        handleRealtimeReactionInserted
      );
      client?.subscribe(
        `a${id}`,
        "reaction_removed",
        handleRealtimeReactionDeleted
      );
      if (!contextual) {
        client?.subscribe(
          `a${id}`,
          "conversation_marked",
          handleRealtimeSeenBy
        );
      }
    }
  ...
  const handleRealtimeReactionInserted = useCallback(
    (realtimeEvent: RealtimeReaction) => {
  ...
  const scrollCallback = () => {
    onNextRender(() =>
      requestAnimationFrame(() =>
        scrollParentToBottom(readMoreRef.current, true)
      )
    );
  };
      ...
      await addMessageMutation.mutateAsync(
        ...
        },
        {
          onSuccess: (data: MessageType) => {
            ...
            // mark conversation as read
            readMessageMutation.mutate(
              { id: id, read: true, messageId: data.id },
              {
                onSettled: () =>
                  onNextRender(() =>
                    requestAnimationFrame(() =>
                      requestAnimationFrame(() =>
                        scrollParentToBottom(messagesEndRef.current, true)
                      )
                    )
                  ),
              }
  ...
  const handleRealtimeMessage = useCallback(
    (realtimeEvent: RealtimeMessage) => {
      if (
        realtimeEvent.message.app_id !== id ||
        realtimeEvent.message.created_by_id === user.id
      )
        return;
      ...
      if (previousData && previousData.pages) {
        ...
        const newPagesArray =
          previousData.pages.map((page: any, i: number) => {
            // add realtime message

            if (i === 0) {
              page.data = [...page.data, realtimeEvent.message];
            }
            return page;
          }) ?? [];
    ...
    <div className="wy-avatar-header">
      {chatRoom && !avatarUrl ? (
        <AvatarGroup
          ...
          size={128}
        />
      ) : !contextual && (displayName || avatarUrl) ? (
        <Avatar src={avatarUrl} name={displayName} size={128} id={id} />
      ) : undefined}
      {!contextual && displayName && (
        <h3 className="wy-headline">{displayName}</h3>
      )}
    </div>
  );
    ...
    <>
      ...
      {data &&
        ...
        data.pages.map((group, i) => {
          ...
          return (
            <React.Fragment key={data.pages.length - i}>
              {group.data?.map((item: MessageType) => {
                ...
                if (lastDate?.format("YYMMDD") !== date.format("YYMMDD")) {
                  lastDate = date;
                  dateContent = (
                    <div className="wy-date-separator">
                      <time>{date.format("L")}</time>
                    </div>
                  );
                }
                ...
                const message = (
                  <Message
                    ...
                    seenBy={
                        !contextual && members.data && members.data.length > 0
                        ? members.data.filter((member) => {
                            return (
                              member.marked_id === item.id &&
                              member.id !== user.id
                            );
                          })
                        : []
  ...
  if (isLoading) {
    messages = <Spinner.UI overlay={true} />;
  }
    ...
    <>
      ...
      <div className="wy-footerbar wy-footerbar-sticky">
        <Editor
          ...
          showMention={hasFeature(
            features,
            Feature.Mentions,
            appFeatures?.mentions
          )}
          showAttachments={hasFeature(
            features,
            Feature.Attachments,
            appFeatures?.attachments
          )}
          showCloudFiles={hasFeature(
            features,
            Feature.CloudFiles,
            appFeatures?.cloudFiles
          )}
          showMeetings={hasFeature(
            features,
            Feature.Meetings,
            appFeatures?.meetings
          )}
